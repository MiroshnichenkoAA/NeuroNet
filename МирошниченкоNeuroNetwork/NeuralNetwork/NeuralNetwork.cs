...
using Miroshnichenko.NeuralNetwork.Layer.InputLayer;
using Miroshnichenko.NeuralNetwork.Layer.OutputLayer;
using Miroshnichenko.NeuralNetwork.LossFunction;
using Miroshnichenko.NeuralNetwork.Neuron;
using Miroshnichenko.NeuralNetwork.WeightsInitializer;

namespace Miroshnichenko.NeuralNetwork
{
        ...
        public void Fit(double[][] inputBatch, double[][] outputBatch, int epochs, double learningRate)
        {
            ...
            for (var epoch = 0; epoch < epochs; epoch++)
            {
                ...
                if (epoch % 10 == 0)
                {
                    ...
                    FormMain.LabelNeurons.Invoke((Action)delegate { FormMain.LabelEp.Text = "Эпоха: " + (epoch+10).ToString(); });
                    FormMain.LabelNeurons.Invoke((Action)delegate { FormMain.LabelNeurons.Text = "Ошибка: " + Math.Round(ev, 5).ToString(); });  
                }
            }
        }
